...
def create_test_data(simulation_parameters):
    ...
    np.savetxt('Test_output/'+str(n_buses)+'/features_test.csv',L_Val, fmt='%s', delimiter=',')
    np.savetxt('Test_output/'+str(n_buses)+'/labels_test.csv',y_gen_data, fmt='%s', delimiter=',')
