...
def create_data(simulation_parameters):
    ...
    #OPF output 
    results = pd.read_csv('Data_File/'+str(n_buses)+'/NN_output.csv').to_numpy()[0:n_data_points][:]
    y_gen_data = results 
    ...
    np.savetxt('Test_output/'+str(n_buses)+'/features_train.csv',L_Val[0:n_data_points][:], fmt='%s', delimiter=',')
    np.savetxt('Test_output/'+str(n_buses)+'/labels_train.csv',np.array(results), fmt='%s', delimiter=',')
